...
Shader "Custom/RippleWater" {
	Properties {
		...
		_Scale ("Scale (cir)", float) = 1
		_Speed ("Speed (cir)", float) = 1
		_Frequency ("Frequency (cir)", float) = 1
		//(Oct 10) Added diagonal waves parameters 
		_Scale_diagonal("Scale (diag)", float) = 1
		_Speed_diagonal("Speed (diag)", float) = 1
		_Frequency_diagonal("Frequency (diag)", float) = 1
  ...
  [HideInInspector]_Distance8 ("Distance8", float) = 0
	...
	SubShader {
		...
		half _Glossiness;
		half _Metallic;
		float _Scale, _Speed, _Frequency;								//circular parameters
		float _Scale_diagonal, _Speed_diagonal, _Frequency_diagonal;	//diagonal parameters
		half4 _Color;
		...
		half offsetvert = ((v.vertex.x * v.vertex.x) + (v.vertex.z * v.vertex.z)); // circular waves
		half offsetvert2 = v.vertex.x + v.vertex.z; //diagonal waves
		half offsetvert3 = v.vertex.x; //horizontal waves, currently not used
		
		//diagonal waves
		half value0 = _Scale_diagonal * sin(_Time.w * _Speed_diagonal * _Frequency_diagonal + offsetvert2);
